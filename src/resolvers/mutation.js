...
module.exports = {
  ...
  deleteNote: async (parent, args, { models, user }) => {
    ...
    const note = await models.Note.findById({ _id: args.id });
    if (note && String(note.author) !== user.id) {
      throw new AuthenticationError(
        'You you do not have permission to delete this note!'
      );
    }
    try {
      await note.remove();
  ...
  updateNote: async (parent, { id, content }, { models, user }) => {
    ...
    if (note && String(note.author) !== user.id) {
      throw new AuthenticationError(
        'You you do not have permission to update this note!'
      );
    }
    try {
  ...
  signIn: async (parent, { username, email, password }, { models }) => {
    ...
    if (!valid) {
      throw new AuthenticationError('Error signing up');
    }
    return jwt.sign({ id: user._id }, process.env.JWT_SECRET);
  },
